...
const Orders: React.FC = () => {
  ...
  const fetchOrders = async () => {
    try {
      const { data, error } = await supabase
        .from("orders")
        .select(`id, order_id, gender, stitch_category, status, due_date, charges, is_completed, created_at, customers (name, phone_number)`)
  ...
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      sewing_seams: "bg-status-sewing", finishing: "bg-status-finishing", completed: "bg-status-completed",
    };
    return colors[status] || "bg-muted";
  };

  const isOverdue = (dueDate: string, isCompleted: boolean) => !isCompleted && isBefore(new Date(dueDate), new Date());
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground">{t("orders.title")}</h1>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value={activeTab} className="mt-4">
          {loading ? (
            ...
            <Card><CardContent className="flex flex-col items-center justify-center py-12"><Filter className="w-12 h-12 text-muted-foreground mb-4" /><p className="text-muted-foreground">{t("orders.noOrders")}</p></CardContent></Card>
          ) : (
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {isOverdue(order.due_date, order.is_completed) && <Badge variant="destructive" className="text-xs">{t("dashboard.overdue")}</Badge>}
                          </div>
